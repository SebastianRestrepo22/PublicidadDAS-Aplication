    ...
    <div className="flex flex-col h-full">
      ...
      <div className="mb-4 flex gap-2">
        <button
          ...
          disabled={cargando || cargandoPermisos}
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
            allSelected
              ? 'bg-blue-600 text-white hover:bg-blue-700'
              : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
          } ${(cargando || cargandoPermisos) ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
      ...
      {/* Lista de permisos con loading */}
      <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar">
        {cargandoPermisos ? (
          <div className="flex flex-col items-center justify-center py-12">
            <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600"></div>
            <p className="mt-3 text-slate-600">Cargando permisos del rol...</p>
          </div>
        ) : (
                          ...
                          {modulePermisos.map((permiso) => {
                            ...
                            return (
                              <div
                                ...
                                onClick={() => !cargando && onPermissionToggle(permiso.PermisoId)}
                              >
